...
public class CaptureTheFlagGame extends Game {
	...
	public void joinGame(Player player) {
		...
		CTFTeam playingTeam = null;
		for (CTFTeam team : teams) {
			if (comparePlayers(team.players, player))
				return;
		...
		sendAllPlayers(ChatColor.AQUA + player.getDisplayName() + " has joined team " + playingTeam.name);
	...
	public void processCommand(CommandContext args, Player player) {
		if (args.argsLength() == 0 || args.getString(0).toLowerCase().equals("getteams")) {
			...
			sb.append(ChatColor.BLUE + "Capture the flag teams");
			for (CTFTeam team : teams) {
		...
		if (args.getString(0).toLowerCase().equals("endgame")) {
			happening = false;
			sendAllGamePlayers(player.getWorld().getPlayers(), ChatColor.BLUE + "Capture the flag has ended.");
			for (CTFTeam team : teams) {
		...
		if (args.getString(0).toLowerCase().equals("setrectangle") || args.getString(0).toLowerCase().equals("sr")) {
			player.sendMessage(ChatColor.BLUE + "The next block you click will be the top left of the rectangle.");
			drawingRectangle = 1;
			return;
		}
		for (CTFTeam team : teams) {
			if (args.getString(0).toLowerCase().equals(team.name.toLowerCase())
				...
				if (args.getString(1).toLowerCase().equals("setflag") || args.getString(1).toLowerCase().equals("sf")) {
					player.sendMessage(ChatColor.GOLD + "The next block you click will be the flag. It will be converted to Iron.");
					team.setSpawn = false;
					team.setFlag = true;
					return;
				}
				...
				StringBuilder sb = new StringBuilder();
				sb.append(ChatColor.BLUE + team.name + " Information:   " + team.points + " Points");
				...
				for (CTFTeam fteam : teams) {
					if (!fteam.name.equals(team.name.toLowerCase())) {
						if (comparePlayers(fteam.players, clickedPlayer)) {
							...
							sendAllPlayers(clickedPlayer.getDisplayName() + " has captured the " + team.name + " flag.");
				...
				if (!comparePlayers(team.players, clickedPlayer)) {
					...
					for (CTFTeam fteam : teams) {
						...
						if (fteam.withFlag != null && fteam.withFlag.getName().equals(clickedPlayer.getName())) {
							...
							sendAllPlayers(clickedPlayer.getDisplayName() + " has captured " + fteam.name + "'s flag.  (" + team.points + " points)");
	...
	public void playerDied(Player player) {
		for (CTFTeam fteam : teams) {
			if (fteam.withFlag.getName().equals(player.getName())) {
				...
				sendAllPlayers(player.getDisplayName() + " has died. The " + fteam.name + " flag has been returned.");
				fteam.withFlag = null;
				return;
			}
		}
				...
				t.schedule(new TimerTask() {
					...
					public void run() {
						if (comparePlayers(fteam.players, player)) {
							...
							sendAllPlayers(player.getDisplayName() + " has rejoined the war.");
						}
					}
				}, waitTime * 1000);
				return;
