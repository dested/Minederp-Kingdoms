package com.minederp.kingdoms.maps;
...
public class ChestWatcherBase implements ContextualMapRenderer {
	...
	public ChestWatcherBase(KingdomsPlugin s) {
		plugin = s;
	}
	...
	public void render(MapView map, MapCanvas canvas, Player player) {
		...
		if (curPlayer.lastFurnaceClicked != null) {
			...
			map.getStringDrawer().drawText(10, 118,
					"CookTime " + curPlayer.lastFurnaceClicked.getCookTime() + " \\ " + curPlayer.lastFurnaceClicked.getLightLevel());

			ItemStack[] cnt = curPlayer.lastFurnaceClicked.getInventory().getContents();
			drawInventory(map, cnt);
		}
		if (curPlayer.lastDispencerClicked != null) {
			map.getStringDrawer().drawText(30, 2, "Dispencer");

			ItemStack[] cnt = curPlayer.lastDispencerClicked.getInventory().getContents();
			drawInventory(map, cnt);
		}

		if (curPlayer.lastChestClicked != null) {
			map.getStringDrawer().drawText(35, 2, "Chest");
			ItemStack[] cnt = curPlayer.lastChestClicked.getInventory().getContents();
			drawInventory(map, cnt);
	...
	private void drawInventory(MapView map, ItemStack[] cnt) {
		...
		for (int i = 0; i < cnt.length; i++) {
			...
			if (item != null) {
				...
				map.getStringDrawer().drawText(x + 3, y + 5, item.getTypeId() + "");
			}
			x += 16;
			if (x > 96) {
				x = 16;
	...
	public void initialize(MapView map) {
		Listener pe = new PlayerListener() {
			...
			public void onPlayerInteract(PlayerInteractEvent event) {
				...
				if ((curPlayer = players.get(event.getPlayer())) == null) {
					players.put(event.getPlayer(), curPlayer = new ListenerObject());
				}

				if (type == Material.CHEST) {
					curPlayer.lastChestClicked = new CraftChest(event.getClickedBlock());
					curPlayer.lastFurnaceClicked = null;
					curPlayer.lastDispencerClicked = null;

				}
				if (type == Material.DISPENSER) {
					curPlayer.lastDispencerClicked = new CraftDispenser(event.getClickedBlock());
					curPlayer.lastFurnaceClicked = null;
					curPlayer.lastChestClicked = null;
				}
				if (type == Material.FURNACE || type == Material.BURNING_FURNACE) {
					curPlayer.lastFurnaceClicked = new CraftFurnace(event.getClickedBlock());
					curPlayer.lastDispencerClicked = null;
					curPlayer.lastChestClicked = null;
		...
		plugin.getServer().getPluginManager().registerEvent(Event.Type.PLAYER_INTERACT, pe, Priority.Normal, plugin);
